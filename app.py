...
def _force_exit_watchdog():
    """
    Watchdog spawned at exit. If PyTorch/EasyOCR leaves non-daemon threads hanging
    after all atexit handlers complete, this will forcefully terminate the process
    after a short grace period.
    """

    def kill_soon():
        logger.info(
            "Shutdown watchdog started, allowing 3 seconds for graceful exit..."
        )
        time.sleep(3.0)
        logger.warning(
            "Graceful exit timed out (likely PyTorch/EasyOCR threads). Force killing process."
        )
        os._exit(0)

    watchdog = threading.Thread(target=kill_soon, daemon=True, name="ExitWatchdog")
    watchdog.start()


# Register watchdog FIRST so it runs LAST in the atexit sequence
...
def get_queue_manager() -> GlobalQueueManager:
    ...
    # Register with atexit for shutdown
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        translate_images = st.checkbox(
            t("translate_images"), value=settings.translate_images
        )
        ...
        with st.expander(t("adv_settings")):
            ...
            preserve_fmt = st.checkbox(
                t("preserve_fmt"), value=settings.preserve_formatting
            )
...
def render_queue_fragment(queue_manager: GlobalQueueManager, session_id: str):
    ...
    with c_actions:
        ...
        with b1:
            completed_jobs = [
                j for j in jobs if j.status == JobStatus.DONE and j.result_data
            ]
            if completed_jobs:
        ...
        with st.container(border=True):
            ...
            with c5:
                ...
                if job.status == JobStatus.PROCESSING:
                    ...
                    with a1:
                        if st.button(
                            "⬆️", key=f"up_{job.id}", disabled=(idx == 0), help="Move Up"
                        ):
                    ...
                    with a3:
                        if st.button(
                            "⏸️", key=f"pause_pend_{job.id}", help=t("btn_pause")
                        ):
                            queue_manager.pause_job(job.id)
                    with a4:
...
def main():
    ...
    # Check for OCR source language restrictions and notify user
    try:
        from services import TranslationService

        ts = TranslationService()
        if (
            hasattr(ts.vision_engine, "was_restricted")
            and ts.vision_engine.was_restricted
        ):
            st.toast(t("ocr_restriction_warn"), icon="⚠️")

        # Ensure OCR model downloads on main thread first
        if "ocr_model_loaded" not in st.session_state:
            with st.spinner(
                "Initializing EasyOCR models (may take a moment to download)..."
            ):
                if (
                    ts.vision_engine.reader is not None
                ):  # This will block and download if needed
                    st.session_state.ocr_model_loaded = True
