...
def sample_region_background(region: Image.Image) -> Tuple[int, int, int]:
    ...
    if w < 2 or h < 2:
        if w >= 1 and h >= 1 and pixels is not None:
            px = pixels[0, 0]
            if isinstance(px, int):
                return (px, px, px)
            return (px[0], px[1], px[2])  # type: ignore
        return (255, 255, 255)
    ...
    if not samples or isinstance(samples[0], int):
        avg = sum(samples) // len(samples)  # type: ignore
        return (avg, avg, avg)

    s_r = sum(s[0] for s in samples) // len(samples)  # type: ignore
    s_g = sum(s[1] for s in samples) // len(samples)  # type: ignore
    s_b = sum(s[2] for s in samples) // len(samples)  # type: ignore
    return (s_r, s_g, s_b)
    ...
    if colors:
        for count, color in colors:
            if isinstance(color, int):
                c_rgb = (color, color, color)
            elif isinstance(color, (tuple, list)):
                c_rgb = (int(color[0]), int(color[1]), int(color[2]))
            else:
                continue

            # Simple Euclidean distance in RGB
            diff = sum((c_rgb[i] - avg_bg[i]) ** 2 for i in range(3)) ** 0.5
            if diff > max_diff:
                max_diff = diff
                text_color = c_rgb
...
def draw_rotated_text(
    ...
    pad = 4
    txt_img = Image.new("RGBA", (int(w + pad * 2), int(h + pad * 2)), (0, 0, 0, 0))
