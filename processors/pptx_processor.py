...
class PptxProcessor(BaseFileProcessor):
    ...
    def load(self, file_path: str | Path) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded PPTX: {path.name} ({len(self._document.slides)} slides)"
            )
    ...
    def extract_content_generator(self) -> Generator[ContentChunk, None, None]:
        ...
        logger.info(
            f"Extracted {len(self._shape_map) + len(self._image_map)} content chunks from PPTX"
        )
    ...
    def _process_shape(
        ...
        shape_prefix = (
            f"{parent_prefix}s{shape_idx}" if parent_prefix else f"s{shape_idx}"
        )
        ...
        if shape.has_table:
            ...
            for row_idx, row in enumerate(table.rows):
                for cell_idx, cell in enumerate(row.cells):
                    ...
                    if cell_text:
                        chunk_id = (
                            f"slide{slide_idx}_{shape_prefix}_t_r{row_idx}_c{cell_idx}"
                        )
                        # Store reference to first paragraph in cell
                        self._shape_map[chunk_id] = (
                            slide_idx,
